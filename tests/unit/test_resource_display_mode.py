"""S72.4 — per-resource netto/brutto price-display override (unit, no DB).

RED→GREEN contract (mirrors the shop/subscription siblings):
- ``BookableResource`` accepts ``price_display_mode`` (``None`` = inherit global,
...
- ``ResourcePricingService.get_resource_pricing_payload`` (S85.2: via the core
  ``PriceFactory``) exposes ``effective_display_mode = override ?? global`` and
  the global ``prices_display_mode`` value itself (so the fe-user consumer can
  render the "netto price" tag).
"""
...
def _resource(price_display_mode=None) -> BookableResource:
...
def test_to_dict_exposes_price_display_mode_default_none():
    resource = _resource(price_display_mode=None)
...
def test_to_dict_exposes_price_display_mode_override():
    resource = _resource(price_display_mode="netto")
...
def test_pricing_payload_stringifies_money_and_keeps_display_mode(monkeypatch):
    ...
    payload = _service().get_resource_pricing_payload(resource)
