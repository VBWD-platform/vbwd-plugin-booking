"""Resource pricing service — computes net/tax/gross via the core ``PriceFactory``.
...
class ResourcePricingService:
    """Compute a resource's net/tax/gross pricing through the ``PriceFactory``."""

    def __init__(self, price_factory):
        """Initialize ResourcePricingService.
    ...
    def get_resource_pricing_payload(self, resource: BookableResource) -> dict:
        ...
        """
        price = self._price_factory.get_price_from_object(resource)
        payload = build_pricing_block(price)
        payload.update(display_mode_fields(resource))
        return payload
